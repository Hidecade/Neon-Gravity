...
let hudFadeTimer = 0;
const HUD_FADE_DURATION = 45; // 45フレームでフェードイン
 ...
 */
function resize() {
    const windowW = Math.round(window.innerWidth);
    const windowH = Math.round(window.innerHeight);

    const isLandscape = windowW >= windowH;
    VIEWPORT.mode = isLandscape ? 'landscape' : 'portrait';
    VIEWPORT.virtualWidth = isLandscape ? 1920 : 1080;
    VIEWPORT.virtualHeight = isLandscape ? 1080 : 1920;

    document.body.classList.remove('portrait', 'landscape');
    document.body.classList.add(VIEWPORT.mode);

    // 目標アスペクト
    const targetAspect = VIEWPORT.virtualWidth / VIEWPORT.virtualHeight;
    ...
    if (windowAspect > targetAspect) {
        fitH = windowH;
        fitW = Math.round(fitH * targetAspect);
    } else {
        fitW = windowW;
        fitH = Math.round(fitW / targetAspect);
    }

    // 整数化を徹底
    GAME_VIEW.width = Math.round(fitW);
    GAME_VIEW.height = Math.round(fitH);
    GAME_VIEW.left = Math.round((windowW - GAME_VIEW.width) / 2);
    GAME_VIEW.top = Math.round((windowH - GAME_VIEW.height) / 2);
    ...
    canvas.style.left = `${GAME_VIEW.left}px`;
    canvas.style.top = `${GAME_VIEW.top}px`;
    canvas.style.width = `${GAME_VIEW.width}px`;
    canvas.style.height = `${GAME_VIEW.height}px`;

    const maxDim = Math.max(width, height);
    deviceViewScale = maxDim / REFERENCE_SIZE;

    if (VIEWPORT.mode === 'landscape') {
        deviceViewScale /= 1.1;
    } else {
        deviceViewScale /= 1.1;
    }

    const screenBaseSize = Math.max(windowW, windowH);
    globalUiScale = screenBaseSize / 800;
    globalUiScale = Math.max(0.8, Math.min(globalUiScale, 2.0));
    const maxAllowedScale = windowW / Math.min(windowW, 800);
    ...
    scaleElements.forEach(el => {
        if (el) {
            el.style.transformOrigin =
                el.id === 'stage-msg' || el.id === 'warning-msg'
                    ? 'center center'
                    : 'top center';
...
function loop() {
    ...
    if (gameState === 'STAGE_INTRO') {
        if (typeof introPhase !== 'undefined' && introPhase === 3) {
            if (hudFadeTimer < HUD_FADE_DURATION) hudFadeTimer++;
        } else {
            hudFadeTimer = 0;
        }
    } else if (gameState === 'PLAYING') {
        hudFadeTimer = HUD_FADE_DURATION;
    } else {
        hudFadeTimer = 0;
    }
...
function draw() {
    ...
    if (
        (gameState === 'PLAYING' ||
            gameState === 'DYING' ||
            (gameState === 'STAGE_INTRO' && typeof introPhase !== 'undefined' && introPhase === 3))
        && typeof drawCanvasHud === 'function'
    ) {
        drawCanvasHud();
    }
}
