...
function handleTouch(e) {
    ...
    // 実サイズ連動
    const stickRadiusL = lR.width / 2;
    const stickRadiusR = rR.width / 2;
    const knobRadiusL = Math.max(20, (ui.knobL.getBoundingClientRect().width / 2));
    const knobRadiusR = Math.max(20, (ui.knobR.getBoundingClientRect().width / 2));

    // 判定半径はスティック半径と同等、移動量はノブ可動範囲
    const activeRadiusL = stickRadiusL;
    const activeRadiusR = stickRadiusR;
    const moveRadiusL = Math.max(16, stickRadiusL - knobRadiusL);
    const moveRadiusR = Math.max(16, stickRadiusR - knobRadiusR);
    ...
    for (let i = 0; i < e.touches.length; i++) {
        ...
        if (dL <= dR) {
            if (dL < activeRadiusL) {
                ...
                const d = Math.min(dL, moveRadiusL);
                input.move.x = Math.cos(a) * (d / moveRadiusL);
                input.move.y = Math.sin(a) * (d / moveRadiusL);
                ui.knobL.style.transform = `translate(${input.move.x * moveRadiusL}px,${input.move.y * moveRadiusL}px)`;
            }
        } else {
            if (dR < activeRadiusR) {
                input.aim.active = true;
                const a = Math.atan2(t.clientY - rC.y, t.clientX - rC.x);
                const d = Math.min(dR, moveRadiusR);
                input.aim.x = Math.cos(a) * (d / moveRadiusR);
                input.aim.y = Math.sin(a) * (d / moveRadiusR);
                ui.knobR.style.transform = `translate(${input.aim.x * moveRadiusR}px,${input.aim.y * moveRadiusR}px)`;
            }
        }
    }
...
function initInputHandlers() {
    ...
    if (ui.controls) {
        ...
        ui.controls.addEventListener('touchend', handleTouchEnd, { passive: false });
        ui.controls.addEventListener('touchcancel', handleTouchEnd, { passive: false });
    }
    if (ui.launchBtn) {
