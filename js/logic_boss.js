...
function updateBattleshipCoreOrbitMovement(e) {
    if (e.coreOrbitDir === undefined) e.coreOrbitDir = Math.random() < 0.5 ? -1 : 1;
    ...
    const ux = dx / dist;
    const uy = dy / dist;
    const tx = -uy * e.coreOrbitDir;
    const ty = ux * e.coreOrbitDir;
    const desiredRadius = 380 * G_SCALE;
    const radialError = dist - desiredRadius;

    const tangentAccel = 0.028 * SPEED_SCALE;
    const radialAccel = Math.max(-0.06, Math.min(0.06, radialError * 0.00022)) * SPEED_SCALE;
    e.vx += tx * tangentAccel - ux * radialAccel;
    e.vy += ty * tangentAccel - uy * radialAccel;

    const maxSpeed = Math.max(e.speed * 3.2, 6.8 * SPEED_SCALE);
    const cv = Math.hypot(e.vx, e.vy);
    if (cv > maxSpeed) {
        e.vx = (e.vx / cv) * maxSpeed;
        e.vy = (e.vy / cv) * maxSpeed;
    }
...
function updateBattleshipAI(e) {
    ...
    if (e.coreExposed) {
        updateBattleshipCoreOrbitMovement(e);
    } else if (isDesperationMode) {
        e.vx *= 0.95; e.vy *= 0.95;
    } else {
