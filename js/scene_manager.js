...
window.skipStory = function () {
    ...
    setTimeout(() => {
        ...
        if (typingMsg) { typingMsg.style.display = 'none'; typingMsg.innerHTML = ''; }
...
async function startEndingSequence() {
    ...
    // ==========================================
    // ★ここに追加：画面が真っ黒になった瞬間に曲をスタート！
    // ピアノソロのイントロがここから流れ始めます。
    // ==========================================
    if (typeof AudioSys !== 'undefined') {
    ...
    // ==========================================
    // 2. すでに画面は真っ黒にフェードアウト済みなので、
    // ここで「1.5秒間の静寂（ピアノソロが流れるタメ）」を作る
    // ==========================================
    await new Promise(resolve => setTimeout(resolve, 1500));

    // 3. 静寂が終わったら、星空と文字をフェードインさせる(ENDING_STORY)
    // ここでドラムのフィルインが鳴り、ギターが爆発するイメージ！
    ...
    // テキストタイピングを開始
    playStoryTyping(ENDING_STORY_TEXT).then(() => {
        // ★大修正：テキスト終了後、すぐには進まない。
        // 代わりに、BGMの終了を待つか、SKIPボタンが押されるのを待つ状態にする。

        // 1. スキップボタンの文字を「NEXT」などに変えて点滅させる（任意ですが、分かりやすいです）
        const skipBtn = document.getElementById('skip-button');
        if (skipBtn) {
            skipBtn.innerText = 'NEXT >>';
            skipBtn.style.animation = 'blink 1.5s infinite';
        }

        // 2. BGMが終了したかを監視するループ（setInterval）をセット
        const checkBgmEnd = setInterval(() => {
            // もしプレイヤーがすでにSKIPボタンを押して次に進んでいたら（isSkipCompleteがtrueなら）、
            // この監視ループは不要なので解除する
            if (isSkipComplete) {
                clearInterval(checkBgmEnd);
                return;
            }

            // BGMが鳴り終わったかを判定
            // AudioSys が存在し、BGMが再生されていない、または終了している場合
            const isBgmPlaying = AudioSys && AudioSys.bgmAudio && !AudioSys.bgmAudio.paused && !AudioSys.bgmAudio.ended;
            ...
            if (!isBgmPlaying) {
                ...
                // 次の画面（リザルト画面など）へ進む
                if (!isSkipComplete) {
                    isSkipComplete = true; // 二重に実行されないようにフラグを立てる
                    showGameOver();
                }
            }
        }, 500); // 0.5秒ごとにチェック
...
function updateCamera() {
    ...
    if (boss && Number.isFinite(boss.x) && Number.isFinite(boss.y)) {
        ...
        targetZoom = 1.0 - (ratio * 0.35 * camT);
    ...
    const finalTargetScale = targetZoom * deviceViewScale;
    ...
    let padX = 150 / deviceViewScale;
    let padY = 125 / deviceViewScale;
